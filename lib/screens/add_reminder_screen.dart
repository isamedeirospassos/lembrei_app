...
class AddReminderScreen extends StatefulWidget {
  final Map<String, dynamic>? lembreteExistente;
  const AddReminderScreen({super.key, this.lembreteExistente});
...
class _AddReminderScreenState extends State<AddReminderScreen> {
  ...
  // ── categoria ─────────────────────────────────────────────
  String _categoriaSelecionada = 'Medicação';

  // ── prioridade ────────────────────────────────────────────
  String _prioridade = 'Media'; // 'Baixa' | 'Media' | 'Alta'

  // ── modo de repeticao ─────────────────────────────────────
  // 'dias' → dias da semana (recorrente)
  // 'data' → data especifica (unico)
  ...
  final List<Map<String, dynamic>> _categorias = [
    {'nome': 'Medicação', 'icone': Icons.medical_services, 'cor': Colors.red},
  ...
  bool get _isEdicao => widget.lembreteExistente != null;
  ...
  void initState() {
    super.initState();
    if (_isEdicao) {
      final l = widget.lembreteExistente!;
      _tituloController.text    = l['titulo']     ?? '';
      _descricaoController.text = l['descricao']  ?? '';
      _categoriaSelecionada     = l['categoria']  ?? 'Medicação';
      ...
      if (l['dataEspecifica'] != null) {
        ...
        _modoRepeticao = 'dias';
        final diasSalvos = List<String>.from(l['dias'] ?? []);
        for (final d in diasSalvos) {
  ...
  // ── salvar ────────────────────────────────────────────────
  void _salvarLembrete() {
    if (_tituloController.text.trim().isEmpty) {
      _snack('digite um titulo para o lembrete!');
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _isEdicao ? 'editar lembrete' : 'novo lembrete',
          ...
          children: [
              ...
              child: Wrap(
                ...
                children: _categorias.map((cat) {
                  ...
                  return GestureDetector(
                    onTap: () =>
                        setState(() => _categoriaSelecionada = cat['nome'] as String),
                    child: AnimatedContainer(
                      duration: const Duration(milliseconds: 200),
              ...
              Row(
                ...
                children: [
                  ...
                  TextButton.icon(
                    ...
                    label: Text(
                      _diasSemana.values.every((v) => v) ? 'limpar' : 'todos',
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                icon: Icon(
                  _isEdicao ? Icons.save : Icons.check,
                  color: Colors.white,
                ),
                label: Text(
                  _isEdicao ? 'salvar edicao' : 'salvar lembrete',
                  style: GoogleFonts.specialElite(
                    color: Colors.white,
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
          ...
          Expanded(
            child: Text(
              algum
                  ...
                  : 'sem dias selecionados — toca apenas hoje',
              style: GoogleFonts.specialElite(
                fontSize: 12,
                color: algum ? Colors.blue[700] : Colors.orange[700],
              ),
